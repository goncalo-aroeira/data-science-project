...
\ctext[RGB]{190,190,190}{Ds 1 has a lot of records, impacting future decisions.

  Not much preprocessing was done. 
  
  The \textit{Age} variable in ds 2 had values with the character "\_", the character was removed. 

  Other anomalies were kept.
}

\subsection*{\textit{Data Dimensionality}}
\ctext[RGB]{190,190,190}{Both datasets have much more records than variables, avoiding curse of dimensionality.
  \par 
...
\ctext[RGB]{190,190,190}{Global boxplots depend on the scaling of variables, in our cases, they are pretty much useless
  \par
...
\ctext[RGB]{190,190,190}{In dataset 1, the variables \textit{LastCheckupTime}, \textit{State} and \textit{AgeCategory} have 
...
\ctext[RGB]{190,190,190}{Binary on binary is not interesting for sparsity evaluation. We can see which variables have more
  impact on the class and the correlation between them.
  \par
  We encoded the symbolic variables into numeric for the correlation analysis. 
  In dataset 1, there are not many variables with high correlation. Even when there are it might not be interesting - gender/height, BMI/weight, Name/ID.
  \par
...
\ctext[RGB]{190,190,190}{\par Dataset 1 - Health \par
...
\ctext[RGB]{190,190,190}{Firstly, we dropped all records with more than 90\% missing values.
...
\ctext[RGB]{190,190,190}{For ds1 the tested strategies significantly worsened the evaluation, particularly the recall, 
...
\ctext[RGB]{190,190,190}{In dataset 1 MinMax does not handle good with outliers, Z-Score outperformed original so chosen was 
Z-Score. For dataset 2, Z-Score outperforms MinMax and original so choice was Z-Score.    
}
...
\ctext[RGB]{190,190,190}{For ds 1 recall was chosen due to the data domain, we want false negatives to be more influencial
...
\ctext[RGB]{190,190,190}{For dataset 1, the SMOTE presented interesting results for recall, but the model was evidently flawed.
...
%% O titulo do grafico dataset1/overfitting esta errado, a distancia n e de manhattan. Por que raio se escolheu
%% k=1, e claramente o que tem mais overfitting e da para ver isso dps na analise do grafico ao lado da matriz????
...
\ctext[RGB]{190,190,190}{For both datasets we limited the search to a maximum depth of 25 due to the computation complexity of the algorithm.
...
\ctext[RGB]{190,190,190}{In this model there is no overfitting for both datasets. The results for each parameter study don't vary much, especially for dataset 2. For the first dataset, with best recall (d, f, nr\_est) = (5, 0.9, 750) and the most important variables are AgeCategory, PhysicalHealthDays and SmokerStatus.
For dataset 2, with best accuracy (d, f, nr\_est) = (7, 0.5, 1000) and the most important variables are OutstandingDebt, InterestRate and CreditMix.}
...
For dataset 2, although the model might be on a slight overfit, especially from the 800th iteration, the high values on all metrics indicate that it's a good model. }
...
\ctext[RGB]{190,190,190}{
  Dataset 2 had better overall results than dataset 1. This might be due to the domain of the data, financial vs health. It is
  harder to quantify health which could lead to less "powerful" data. It is to note that dataset 1 had nearly 4 times more
  records than dataset1, nonetheless, it performed worse across all metrics across all different models. \par

  In dataset 1 we ended up not choosing any measure to deal with outliers as it was what gave us better metrics. KNN and NB
  are particullarly sensitive to their existence. Despite this, we performed Z-Score scaling on both our datasets, which
  mitigates the outliers issue of especially dataset 1.

  \par Best model for ds 1: NB  (GaussianNB)
  \par Best model for ds 2: KNN (k=3 and Manhattan distance), it has the same accuracy as gradient boosting but the 
  test accuracy is closer to the training one, indicating less overfitting on KNN.

  \par Worst model for ds1: KNN (k=1 and Chebyshev distance), it has huge overfitting % talvez va ser preciso mudar esta treta
  \par Worst model for ds2: NB (BernoulliNB)

  GB most important variable was AgeCategory for health dataset and OutstandingDebt for the services one. In Random Forests
  it was the same case and so was in Decision Trees. This leads to the conclusion that these variables were the most interesting
  in their respective datasets.

  The KNN results clearly illustrate the fact that overfitting is greater at smaller values of k as this can be observed in 
  both datasets.

  RF and DT dont like Z-Score very much. Helps convergence so GB and NN.
  DT benefit from SMOTE balancing. As well as KNN, without it the model would skew towards the predominant class. 
  SMOTE helps improve recall.

  The models that showed most overfitting were DT and GB.
  Per example, use hold-out or cross-validation during training, better feature enginnering.

  Other than the MLP for dataset 1 being much worse than his counterparts, the different models didn't differ much in 
  terms of evaluation metrics from one another, especially recall for dataset 1 and accuracy for dataset 2. This could be
...
\ctext[RGB]{190,190,190}{We used the "sum" function as "agg\_fun" for both datasets. We studied the granularity at three different levels, for dataset 1, weekly (atomic), monthly and quarterly with an upwards trend but no seasonality or cyclical behaviour. 

For dataset 2 by 15 minutes (atomic), hourly and daily, with no visible trend, daily seasonality on each morning and evening and also weekly cyclicality with a busier day a week (usually Mondays except the first spike) both corresponding to heavier traffic flows. }
...
\ctext[RGB]{190,190,190}{For the 1st dataset we obtained a p-value of 0.223. Looking at the graphs, we can see that there is a trend in the first 2 and that there is evidence of a seasonal trend in the 3rd. 

For the 2nd dataset we obtained a p-value of 0. and that there is no trend but there is some seasonality.}
...
\ctext[RGB]{190,190,190}{To study the best aggregation, we applied the model at 3 different levels studied in Data Profiling. 

In dataset 1, chose the weekly aggregation and for dataset 2, hourly aggregation as they obtained lower values for the different errors, simplifying the model without losing information or context.}
...
\ctext[RGB]{190,190,190}{To study the best Window Size, we applied the model to 4 different values (25, 50, 75 and 100). 

In this case we chose 100 for both datasets since it was where we obtained the lowest values for the different errors.}
...
\ctext[RGB]{190,190,190}{To study differentiation, we applied the first two derivatives, favoring the first one for both datasets.

In dataset 1, it helped remove quadratic trends and minimize the errors and for dataset 2, both derivatives removed seasonality, but the second added complexity, making it harder to predict.}
...
\ctext[RGB]{190,190,190}{Finally, we applied scaling in both datasets in order to have best values to use in the models' evaluation specifically in the LSTM model.  }
...
\section{MODELS' EVALUATION}
\ctext[RGB]{190,190,190}{For dataset 1 we used the weekly aggregation with window size=100, the first derivative and then applied scaling. 
...
\ctext[RGB]{190,190,190}{Dataset 1, with a trend-style pattern, benefits from parameters (p, d, q) set to (7, 2, 5). This configuration allows the model to capture and accommodate the complexities associated with trend-based data.
