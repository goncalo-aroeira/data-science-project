...
\ctext[RGB]{190,190,190}{For dataset 1, the SMOTE presented interesting results for recall, but the model was evidently flawed.
  ...
  \par
  We chose SMOTE balancing transformation for ds 2 as the others are not suited for smaller datasets and it is the
  one that allows to preserve more information.
  \par
  We also from the profiling that both datasets required some form of balancing.
}
...
\ctext[RGB]{190,190,190}{
  ...
  harder to quantify health. It is to note that dataset 1 had nearly 4 times more
  records than dataset1, nonetheless, it still performed worse across all metrics for all different models. \par

  In dataset 1 we didn't choose any measure to deal with outliers as it was what gave us better metrics. KNN and NB
  ...
  \par Worst model for ds1: KNN (k=1 and Chebyshev distance), it has huge overfitting
  ...
  The models that showed most overfitting were DT and GB. Fix to DT would be lower depth.
