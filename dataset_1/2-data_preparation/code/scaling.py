...
scaling_evaluation("../data/CovidPos_scaled_minmax.csv", "MinMax1", target)

#Approach 2 - Minmax Scaler2 [0,10]
transf: MinMaxScaler = MinMaxScaler(feature_range=(-1, 1), copy=True).fit(data)
df_minmaxRange = DataFrame(transf.transform(data))
df_minmaxRange[target] = target_data
df_minmaxRange.columns = vars
df_minmaxRange.to_csv(f"../data/CovidPos_scaled_minmaxRange.csv", index=False)
print(f"Data after minmax scaling: {df_minmaxRange.head(20)}")

scaling_evaluation("../data/CovidPos_scaled_minmaxRange.csv", "MinMax2", target)
